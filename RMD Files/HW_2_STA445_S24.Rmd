...
vec_d + vec_a
```
It appears that R repeated added the values in vector d into vector a, adding vector_d[0] = 14 to vector_a[0] and vector_a[3].  The error was due to the longer vector not being an integer multiple of the shorter vector (i.e. it was able to add the first value of vector d into vector a twice, but the second value of vector d was only added into vector a once, not twice as well).
...
vec_d + vec_a
```
That is because the longer vector is an integer multiple of the shorter vector.
...
```{r}
vec_prob4 <- seq(from=1, to=5, by=1)
vec_prob4
```
...
```{r}
vec_prob5a <- seq(from=2, to=20, by=2)
vec_prob5a
```
...
df[  which( !is.na(df$Grade) ), ]
```
The first method is forcing the negative value of the bool evaluation by essentially multiplying the result by -1.

The second method is using the bang logical operator (!) within the evaluation to ask if the value at df$Grade is NOT NA.
