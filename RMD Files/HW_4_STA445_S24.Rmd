...
microbenchmark::microbenchmark( duniform( seq(-4,12,by=.0001), 4, 8), times=100)
```
The ifelse function is both easier to write out and is also faster; however, it is not as readable as using a for loop since one would not know how the ifelse function works without knowing a bit about it first.
